...
```
(base) ➜  drlnd-p3 git:(master) ✗ conda create --name drlnd-p3 python=3.6
(base) ➜  drlnd-p3 git:(master) ✗ conda activate drlnd-p3
```
...
Start a `jupyter notebook` from within the project folder and follow the instructions in `notebooks/Tennis.ipynb` to either
